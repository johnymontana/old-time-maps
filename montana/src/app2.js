/* =====================================================================
   Montana in Relief — Part 2: camera, labels, instruments, tools, tours
   ===================================================================== */
(function(){
'use strict';
const X = window.MTX, M = X.M, $ = X.$;
const {clamp, lerp, DEG, W, H, RELIEF, uvToLonLat, wx, wz, ux, vz, U} = X;
let renderer, scene, camera;

/* ================= camera rig ================= */
const cam = {
  tx:0, ty:0, tz:0, az:168*DEG, el:32*DEG, dist:1080,
  Tx:0, Ty:0, Tz:0, Taz:168*DEG, Tel:32*DEG, Tdist:1080
};
const HOME = {az:168*DEG, el:32*DEG, dist:1080, x:0, z:0};
function fitHome(){
  if(!camera) return;
  ...
  const dH = (along*0.60*Math.sin(HOME.el) + RELIEF*5*0.6)/Math.tan(vf/2);
  HOME.dist = clamp(Math.max(dW,dH), 380, 2600);
}
function applyCam(){
...
function damp(dt){
  ...
  const gy = X.terrainY(cam.Tx, cam.Tz);
  cam.Ty = gy;
  cam.ty += (cam.Ty-cam.ty)*k;
}
function clampTarget(){
  cam.Tx = clamp(cam.Tx, -W*0.56, W*0.56);
  cam.Tz = clamp(cam.Tz, -H*0.58, H*0.58);
  cam.Tdist = clamp(cam.Tdist, 9, 2600);
...
function pickAt(cx, cy){
  ...
  const maxY = RELIEF*X.getExag()+0.5;
  let t=0;
  if(o.y>maxY){ if(d.y>=-1e-6) return null; t=(o.y-maxY)/(-d.y); }
  let step = Math.max(0.35, cam.dist*0.0035);
  const base = U.uBase.value;
  for(let i=0;i<900;i++){
    t += step; step *= 1.010;
    const x=o.x+d.x*t, y=o.y+d.y*t, z=o.z+d.z*t;
    if(y < base-2 && d.y<0) return null;
    if(t > 9000) return null;
...
function onMove(e){
  ...
  if(pinch && ptrs.size===2){
    ...
    cam.Tdist = clamp(pinch.dist*pinch.d/Math.max(d,1), 9, 2600);
...
function panBy(dx,dy){
  ...
  // screen right = (cos az, 0, sin az); screen "up" on ground = (-sin az, 0, cos az)
...
function onWheel(e){
  ...
  cam.Tdist = clamp(cam.Tdist*f, 9, 2600);
  if(f<1){
...
const show = {peak:true, city:true, feature:true};
let labelFade = 0;
...
function occluded(x,y,z){
  ...
  for(let i=1;i<=14;i++){
    ...
    if(py < X.terrainY(px,pz)-0.05) return true;
  }
  return false;
}
let lblTick=0;
function updateLabels(){
  ...
  const boxes=[];
  const cand=[];
  const lift = 0.5 + d*0.004;
  for(const L of labels){
    let on = show[L.kind];
    if(on && L.kind==='city'){
      const t = L.prio;
      on = t>=9 || (t>=6 && d<1050) || (t<6 && d<520);
    }
    if(on && L.kind==='feature') on = d<1500 && d>26;
    ...
    if(_p3.z>1 || _p3.x<-1.2||_p3.x>1.2||_p3.y<-1.2||_p3.y>1.2){
      if(L.vis){L.el.style.opacity='0'; L.vis=false;} continue;
    }
    if(full && occluded(L.x,y,L.z)){ if(L.vis){L.el.style.opacity='0'; L.vis=false;} continue; }
...
mini.addEventListener('pointerdown', e=>{
  ...
  cam.Tdist=Math.min(cam.Tdist, 420); clampTarget();
});
...
function setMarker(p){
  ...
  measure.mark.scale.setScalar(clamp(cam.dist*0.006,0.6,6));
}
...
function drawProfile(hoverT){
  ...
  const nD=Math.min(6,Math.max(2,Math.round(s.len/60)));
  for(let i=0;i<=nD;i++){
    const t=i/nD;
...
profcv.addEventListener('pointermove',e=>{
  ...
  const x=s.pts[i*2], z=s.pts[i*2+1];
  setMarker({x, y:(s.hs[i]-X.HMIN)/1000*X.getExag()+0.4, z});
});
...
/* ================= tours ================= */
const tour={active:false, t:0, keys:null, i:0, id:null};
const SEG=5.6, HOLD=1.5;
function exFor(d){ return clamp(1.7+d*0.021, 2.1, 5.6); }
function setExagUI(v){
  X.setExag(v);
  const el=$('#exag'); el.value=Math.round(v*10);
  $('#exagv').textContent='×'+v.toFixed(1);
}
function keyToCam(k){
...
function tickTour(dt){
  ...
  $('#tourprog').style.width=clamp(tour.t/total,0,1)*100+'%';
  ...
  const A=K[seg], B=K[Math.min(seg+1,n-1)];
  let f=clamp(t/SEG,0,1);
  f=f*f*f*(f*(f*6-15)+10);
  const ease=f;
  cam.tx=lerp(A.x,B.x,ease); cam.tz=lerp(A.z,B.z,ease);
  cam.dist=lerp(A.d,B.d,ease);
  let da=((B.az-A.az+Math.PI*3)%(Math.PI*2))-Math.PI;
  cam.az=A.az+da*ease;
  cam.el=lerp(A.el,B.el,ease);
  setExagUI(lerp(A.ex,B.ex,ease));
  cam.ty=X.terrainY(cam.tx,cam.tz);
  const idx = (t>SEG*0.55 && seg+1<n) ? seg+1 : seg;
...
function wireUI(){
  ...
  }, v=> v>=97?'1991 sheet':(v<=3?'Modern relief':v+'% sheet'));
  bindSlider('exag', v=>{ X.setExag(v/10); rebuildProfileGeom(); }, v=>'×'+(v/10).toFixed(1));
  ...
  bindToggle('tPeaks', true, v=>{show.peak=v;});
  ...
  $('#btnPlan').addEventListener('click',e=>{
    ...
    if(planMode){
      ...
      cam.Tel=89.4*DEG; cam.Taz=0;
      setSlider('mix',100); setSlider('shade',18); setSlider('exag',8);
    } else if(planSave){
...
function tick(){
  ...
  if(intro.on){
    ...
    if(f>=1){ intro.on=false; labelFade=1; $('#exag').value=50; }
...
window.MTdebug=()=>({fade:labelFade, intro:intro.on, dist:cam.dist, az:cam.az/DEG, el:cam.el/DEG,
  ty:cam.ty, camY:camera.position.y, nlbl:labels.length, step:perf.step,
  sample:labels.slice(0,3).map(L=>({n:L.el.textContent.slice(0,14), y:X.terrainY(L.x,L.z),
     occ:occluded(L.x, X.terrainY(L.x,L.z)+0.5+cam.dist*0.004, L.z), op:L.el.style.opacity, tr:L.el.style.transform}))});
...
(async function main(){
  ...
  buildLabels(); buildTours(); buildMini(); wireUI(); updateSun();
  X.setExag(0.02);
  applyCam();
  renderer.render(scene,camera);
